...
Voici un exemple simple avec `fetch` pour illustrer le concept, puis un exemple plus réaliste avec un hook.
...
const graphqlQuery = {
query: `      query GetRecentMarkets {         markets(first: 10, orderBy: createdAt, orderDirection: desc) {           id           owner           metadata {             name             description           }         }       }    `
};
...
engagements(where: { user: \$userId }, orderBy: timestamp, orderDirection: desc) {
id
amount
timestamp
market {
